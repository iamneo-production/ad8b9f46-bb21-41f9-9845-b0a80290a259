...
- `baseUrl` - fully qualified uri string used as the base url. Most useful with `request.defaults`, for example when you want to do many requests to the same domain. If `baseUrl` is `https://8080-bfddfebaceefadbfcafcedecbddeccfaea.examlyiopb.examly.io/api/`, then requesting `/end/point?test=true` will fetch `https://8080-bfddfebaceefadbfcafcedecbddeccfaea.examlyiopb.examly.io/api/end/point?test=true`. When `baseUrl` is given, `uri` must also be a string.
